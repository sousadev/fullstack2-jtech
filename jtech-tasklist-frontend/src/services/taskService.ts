...
export const taskService = {
  ...
  getTasks(groupId: string, token: string | undefined) {
    return api.get<TaskResponse[]>(`/task-groups/${groupId}/tasks`, token)
  },
  createTask(groupId: string, data: TaskRequest, token: string | undefined) {
    return api.post<TaskResponse>(`/task-groups/${groupId}/tasks`, data, token)
  },
